...
export function mountVR() {
  ...
  // (async () => {
  //   console.log('[XR] navigator.xr:', !!navigator.xr, location.protocol);
  //   if (navigator.xr && navigator.xr.isSessionSupported) {
  //     try {
  //       const ok = await navigator.xr.isSessionSupported('immersive-vr');
  //       console.log('[XR] immersive-vr supported?', ok);
  //       if (!ok) {
  //         // 即使不支援，VRButton 仍會顯示「VR NOT SUPPORTED」，若沒有顯示，多半是環境/載入錯
  //       }
  //     } catch (e) {
  //       console.warn('[XR] isSessionSupported error:', e);
  //     }
  //   } else {
  //     console.warn('[XR] WebXR not available (secure context? runtime? browser?)');
  //   }
  // })();
  ...
  //     fake.id = 'VRButton';
  //     fake.textContent = 'VRButton (diagnostic)';
  //     fake.style.cssText = 'position:fixed;top:20px;right:20px;z-index:9999';
